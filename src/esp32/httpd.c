...
static void httpd_send_chunked(const char* buffer,
                               size_t buffer_len, void* response_state) {
    char buf[64];
    if (buffer) {
        if(buffer_len) {
            itoa(buffer_len, buf, 16);
            strcat(buf, "\r\n");
            httpd_send_block(buf,strlen(buf), response_state);
            httpd_send_block(buffer,buffer_len, response_state);
            httpd_send_block("\r\n",2, response_state);
        }
        return;
    }
...
void httpd_send_expr(const char* expr, void* response_state) {
    httpd_send_chunked(expr, strlen(expr), response_state);
}
